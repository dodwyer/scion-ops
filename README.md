...
For the default image-based Hub update path, first run `task build:base` to
rebuild `localhost/scion-base:latest`, then run `task update:hub` to load that
image into kind and restart only the Hub deployment.
