...
Update `state.json` after every phase transition, child completion, review
verdict, validation run, blocker, and final decision. The state file is the
source of truth for automation that watches or resumes the session.
...
- Child implementers own their own branches. The final implementation branch is
  the integration branch named in the task prompt.
...
messages and copy durable facts into `state.json`.
