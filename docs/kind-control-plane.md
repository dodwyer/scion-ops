...
Open the web app browser interface directly at `http://192.168.122.103:8787` after
`task up`. No port-forwarding is required.

The MCP pod and web app read Hub through the in-cluster `scion-hub` Service and
use the `scion-hub-dev-auth` Secret restored by `task bootstrap`.
...
the web app Deployment, and reports rollout status and service information.

Web app status and logs:
