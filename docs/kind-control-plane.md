...
scion-ops runs Scion Hub, Runtime Broker, MCP, the read-only web app, and
agent pods in Kubernetes.
...
Kind exposes Hub, MCP, and the web app through native port mappings and
NodePort Services. Do not use `kubectl port-forward` for normal operation.
...
The web app uses the same in-cluster Hub endpoint, MCP service URL, mounted
workspace, dev-auth Secret, and optional GitHub token Secret. It is read-only:
browser and API POST/PUT/PATCH/DELETE requests return `405`.
...
the web app HTML/API/read-only endpoint, and no-auth Kubernetes agent dispatch.
