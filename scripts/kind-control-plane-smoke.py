...
DEFAULT_WEB_APP_URL = os.environ.get("SCION_OPS_WEB_APP_URL", "http://192.168.122.103:8787")
...
def ensure_web_app(url: str, *, timeout_seconds: int) -> None:
    """Verify the web app HTTP endpoint responds and returns a valid snapshot."""
    ...
    while time.monotonic() <= deadline:
        try:
            ...
            with urllib.request.urlopen(req, timeout=4) as resp:
                if resp.status < 500:
                    body = resp.read(1024)
                    log(f"kind web app is reachable at {url} (HTTP {resp.status})")
                    return
...
async def smoke(args: argparse.Namespace) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="scion-kind-cp-smoke-"):
        try:
            ...
            if not args.skip_web_app and not args.skip_mcp:
                ensure_web_app(args.web_app_url, timeout_seconds=args.startup_timeout)
            ...
            print(f"  web-app:    {'skipped' if args.skip_web_app or args.skip_mcp else args.web_app_url}")
