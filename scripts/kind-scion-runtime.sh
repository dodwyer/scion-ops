...
WEB_HOST_PORT="${SCION_OPS_WEB_PORT:-8787}"
HUB_NODE_PORT="30090"
MCP_NODE_PORT="30876"
WEB_NODE_PORT="30878"
...
Environment:
  ...
  SCION_OPS_WEB_PORT             Host port for the web app service (default: 8787)
...
render_cluster_config() {
  ...
  sed \
    ...
    -e "s|__WEB_NODE_PORT__|$(sed_replacement_escape "$WEB_NODE_PORT")|g" \
    -e "s|__WEB_HOST_PORT__|$(sed_replacement_escape "$WEB_HOST_PORT")|g" \
...
Required mappings:
  ...
  ${KIND_LISTEN_ADDRESS}:${WEB_HOST_PORT} -> kind node ${WEB_NODE_PORT} -> scion-ops-web
EOF
}
...
cmd_status() {
  ...
  printf 'Web host URL: http://%s:%s\n\n' "$KIND_LISTEN_ADDRESS" "$WEB_HOST_PORT"
